 ...
 *  判断经纬度是否超出中国境内
 */
export function isLocationOutOfChina(latitude: number, longitude: number) {
  if (
    longitude < 72.004 ||
    longitude > 137.8347 ||
    latitude < 0.8293 ||
 ...
 */
export function transformFromWGSToGCJ(latitude: number, longitude: number) {
  var lat = 0;
  var lon = 0;
 ...
 *  将GCJ-02(火星坐标)转为百度坐标:
 */
export function transformFromGCJToBaidu(latitude: number, longitude: number) {
 ...
 *  将百度坐标转为GCJ-02(火星坐标):
 */
export function transformFromBaiduToGCJ(latitude: number, longitude: number) {
 ...
 */
export function transformFromGCJToWGS(latitude: number, longitude: number) {
...
function isContains(point: any, p1: any, p2: any) {
  return (
...
function transformLatWithXY(x: number, y: number) {
  var pi = 3.14159265358979324;
  var lat =
    -100.0 +
    2.0 * x +
...
function transformLonWithXY(x: number, y: number) {
  var pi = 3.14159265358979324;
  var lon =
    300.0 +
    x +
